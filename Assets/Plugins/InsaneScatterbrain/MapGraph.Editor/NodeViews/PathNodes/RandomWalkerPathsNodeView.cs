...
{
    [ScriptNodeView(typeof(RandomWalkerPathsNode))]
    public class RandomWalkerPathsNodeView : ScriptNodeView
    {
        public RandomWalkerPathsNodeView(RandomWalkerPathsNode node, ScriptGraphView graphView) : base(node, graphView)
        {
            this.AddPreview<RandomWalkerPathsNode>(GetPreviewTexture);
        }

        private Texture2D GetPreviewTexture(RandomWalkerPathsNode node)
        {
            var xMax = 0;
            var yMax = 0;
